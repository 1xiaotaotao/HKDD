...
class ExtractFeaturesEsm1b:
	...
	def extract(self):
		self.logger.info('extracting esm1b features')
		...
		self.logger.info('extracting features of pos by esm1b')
		pos_features = self.extract_process(pos_sequences)
		self.logger.info('extracting features of neg by esm1b')
		neg_features = self.extract_process(neg_sequences)
		return torch.cat([pos_features, neg_features], dim=0)  # [n, 512, 1280]
